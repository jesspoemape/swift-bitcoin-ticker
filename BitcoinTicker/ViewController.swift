...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func updateBitcoinData(json : JSON) {
        let dirtyBitcoinResult = json["ask"].doubleValue
        let cleanBitcoinResult = dirtyBitcoinResult.truncate(to: 2)
        
        bitcoinPriceLabel.text = "\(selectedCurrencySymbol)\(cleanBitcoinResult)"
    }
